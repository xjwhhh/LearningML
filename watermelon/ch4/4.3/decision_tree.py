...
def TreeGenerate(df):
    ...
    if label_count:  # assert the label_count isn't empty
        ...
        # end if there is only 1 class in current node data 样本全属于同一类别
        # end if attribution array is empty 属性集为空
        if len(label_count) == 1 or len(label_arr) == 0:
            return new_node

        # get the optimal attribution for a new branching 获取最优划分属性
...
采用信息增益最大化来实现最优划分属性的选择，这里主要的挑战是离散和连续两种属性变量的分别操作。
对于离散变量（categorical variable），参考书p75-77内容实现，
